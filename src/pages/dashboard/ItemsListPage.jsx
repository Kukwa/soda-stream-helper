...
export function ItemsListPage() {
  ...
  const getItemsByCategory = async (householdId, category, onItemsChange) => {
    const itemsQuery = query(
      collection(firestore, `households/${householdId}/items`),
    ...
    onSnapshot(itemsQuery, async (snapshot) => {
      ...
      const itemList = snapshot.docs.map(async (doc) => {
        const item = doc.data();
        item.image = await getDownloadURL(
          ref(storage, doc.ref.path.toString())
        );
        item.id = doc.id;
        return item;
      });
  ...
  const handleNewItemConfirm = async () => {
    ...
    const newItemRef = doc(
      collection(firestore, `households/${MOCK_HOUSEHOLD_ID}/items`)
    );
    try {
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={MODAL_TITLES_BY_CATEGORY[newItemCategory]}
        centered
      ...
      >
        ...
        />
        {newItemImage && (
          <Image radius="xs" src={URL.createObjectURL(newItemImage)} />
        )}
        <Space h="md" />
        <Center>
          <FileButton
        ...
        {isCompleted && (
          <Center>
            <Button onClick={handleNewItemConfirm} loading={isNewItemLoading}>
              {" "}
              Add new Item{" "}
            </Button>
          </Center>
      ...
      <ItemListComponent
        items={bottleList}
        itemHeader={"Your Bottles"}
        onAddClick={onBottleItemClick}
      />
      <Space h="xl" />
      {/*Syrups*/}
      <ItemListComponent
        items={syrupList}
        itemHeader={"Your Syrups"}
        onAddClick={onSyrupItemClick}
      />
      {/*Gas cylinders*/}
      <ItemListComponent
        items={cylinderList}
        itemHeader={"Your Gas Cylinders"}
        onAddClick={onCylinderItemClick}
      />
    </>
  );
}
